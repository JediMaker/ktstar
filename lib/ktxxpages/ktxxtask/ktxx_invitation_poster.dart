...
class KeTaoFeaturedInvitationPosterPage extends StatefulWidget {
  ...
  @override
  _KeTaoFeaturedInvitationPosterPageState createState() => _KeTaoFeaturedInvitationPosterPageState();
}
// Copyright (c) 2021, the Dart project authors.  Please see the AUTHORS file
// for details. All rights reserved. Use of this source code is governed by a
// BSD-style license that can be found in the LICENSE file.
class _KeTaoFeaturedInvitationPosterPageState extends State<KeTaoFeaturedInvitationPosterPage> {
  ...
  void _initData() async {
    var result = await KeTaoFeaturedHttpManage.getInvitationPosters();
    if (result.status) {
      if (mounted) {
        setState(() {
...
//                                        color: Colors.white,
  ...
  _savePosterImg() async {
    try {
      var response = await KeTaoFeaturedHttpManage.dio.get(_bannerList[_bannerIndex],
