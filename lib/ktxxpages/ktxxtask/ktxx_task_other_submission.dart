...
class KeTaoFeaturedTaskOtherSubmissionPage extends StatefulWidget {
  ...
  @override
  _KeTaoFeaturedTaskOtherSubmissionPageState createState() =>
      _KeTaoFeaturedTaskOtherSubmissionPageState();
}

class _KeTaoFeaturedTaskOtherSubmissionPageState extends State<KeTaoFeaturedTaskOtherSubmissionPage> {
  ...
  _initData() async {
    if (widget.pageType == 0) {
      var result = await KeTaoFeaturedHttpManage.getTaskOtherSubmitInfo(widget.taskId);
      if (mounted) {
        if (result.status) {
          setState(() {
      ...
      var result = await KeTaoFeaturedHttpManage.getTaskOtherReSubmitInfo(
    ...
    return Container(
      ...
      child: GestureDetector(
        onTap: () async {
          ...
          if (widget.pageType == 0) {
            ///提交任务
            var result = await KeTaoFeaturedHttpManage.taskReSubmit(widget.comId, imgIds,
                remark: _remark);
            if (result.status) {
