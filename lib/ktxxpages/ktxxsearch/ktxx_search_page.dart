...
class _KeTaoFeaturedSearchGoodsPageState extends State<KeTaoFeaturedSearchGoodsPage>
  ...
  _getHotWordsData() async {
    ...
    var result = await KeTaoFeaturedHttpManage.getHotSearchWords();
    try {
      EasyLoading.dismiss();
    } catch (e) {}
    if (result.status) {
...
*/
  ...
  buildPopularSearches() {
    if (KeTaoFeaturedCommonUtils.isEmpty(_popularProductWords)) {
  ...
  Widget getHistoryWidget() {
    if (KeTaoFeaturedCommonUtils.isEmpty(_hisArray)) {
      return SliverToBoxAdapter();
    }
    return SliverToBoxAdapter(
      child: Visibility(
  ...
  Widget buildSearchPddProductList() {
    return SliverToBoxAdapter(
      child: Column(
        children: [
          Visibility(
            visible: !KeTaoFeaturedCommonUtils.isEmpty(pddGoodsList) &&
                _showSearchList &&
                _selectedTabIndex == 1,
            child: Center(
              child: Container(
