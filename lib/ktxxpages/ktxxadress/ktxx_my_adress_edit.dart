...
import 'package:star/ktxxpages/ktxxwidget/ktxx_jd_address_selector.dart';
import 'package:star/ktxxutils/ktxx_common_utils.dart';

import '../../ktxx_global_config.dart';
import 'ktxx_my_adress.dart';
//  return Column(
...
class KeTaoFeaturedAddressDetailPage extends StatefulWidget {
  ...
  @override
  _KeTaoFeaturedAddressDetailPageState createState() => _KeTaoFeaturedAddressDetailPageState();
}
// Copyright (c) 2021, the Dart project authors.  Please see the AUTHORS file
// for details. All rights reserved. Use of this source code is governed by a
// BSD-style license that can be found in the LICENSE file.
class _KeTaoFeaturedAddressDetailPageState extends State<KeTaoFeaturedAddressDetailPage>
  ...
  Future _initData() async {
    ...
    var resultData =
        await KeTaoFeaturedHttpManage.getShippingAddressDetail(widget.addressId);
    if (mounted) {
      setState(() {
        ...
        actions: <Widget>[
          GestureDetector(
            ...
            onTap: () async {
              ...
              if (widget.addressId != null) {
                var result = await KeTaoFeaturedHttpManage.modifyShippingAddress(
                ...
                var result = await KeTaoFeaturedHttpManage.addShippingAddress(
                  consignee: _unameController.value.text,
                  mobile: _phoneController.value.text,
                  address: _addressDetailController.value.text,
                  provinceId: _provinceId,
            ...
            children: <Widget>[
              ...
              Visibility(
                child: Ink(
                  child: GestureDetector(
                    onTap: () async {
                      var result = await KeTaoFeaturedHttpManage.deleteShippingAddress(
                          widget.addressId);
                      if (result.status) {
                        Fluttertoast.showToast(
                            msg: "地址删除成功！",
